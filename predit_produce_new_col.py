...
#創造3個新欄位(計算每個瀏覽頁面平均瀏覽時間)Administrative_Duration/Administrative, Informational_Duration/Informational, ProductRelated_Duration/ProductRelated
train_new[19] = train_new.apply(lambda x :x[2]/x[1],axis=1)
train_new[20] = train_new.apply(lambda x :x[4]/x[3],axis=1)
train_new[21] = train_new.apply(lambda x :x[6]/x[5],axis=1)
print(train_new)

#若除數為0(頁面瀏覽數量為0)，計算結果為NA，代表未實際使用頁面，故將NA值補0
train_new = train_new.fillna(value=0)
print(train_new)

#將Revenue移動到最後一欄
move_col = train_new.pop(train_new.columns[-4])
train_new.insert(21, move_col.name, move_col)
print(train_new)

#重新定義欄位索引
train_new.columns = [i for i in range(22)]
print(train_new)

#儲存清洗後之train資料集為CSV檔
train_new.to_csv(r'C:\Users\88691\PycharmProjects\hotel_sales\data-question\train_new_col.csv', index = False)

#分配train資料集
X_train, X_test, y_train, y_test = train_test_split(train_new.iloc[1:, 1:21], train_new.iloc[1:, 21:],
...
#創造3個新欄位(計算每個瀏覽頁面平均瀏覽時間)Administrative_Duration/Administrative, Informational_Duration/Informational, ProductRelated_Duration/ProductRelated
test_new[18] = test_new.apply(lambda x :x[2]/x[1],axis=1)
test_new[19] = test_new.apply(lambda x :x[4]/x[3],axis=1)
test_new[20] = test_new.apply(lambda x :x[6]/x[5],axis=1)
print(test_new)

#若除數為0(頁面瀏覽數量為0)，計算結果為NA，故將NA值補0
test_new = test_new.fillna(value=0)
print(test_new)

#儲存清洗後之test資料集為CSV檔
test_new.to_csv(r'C:\Users\88691\PycharmProjects\hotel_sales\data-question\test_new_col.csv', index = False)
...
stack_list[7].fit(train_new.iloc[:,1:21],train_new.iloc[:,21:])
...
y_submit.to_csv(r'C:\Users\88691\PycharmProjects\hotel_sales\Submission_col.csv', index = False)
